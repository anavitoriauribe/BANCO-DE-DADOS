...
--- 
```sql
DROP DATABASE uribemarket; --resulta em *erro* se caso não exitir o banco de dados
```

```sql
DROP DATABASE IF EXISTS uribemarket; --resulta em uma *noticia* se caso não existir
```
![alt text](image.png)
...
## Limitar quantidade de produtos à mostra:
```sql
 SELECT * FROM produtos LIMIT 5;
 ```

 ## Mostra as categorias do banco de dados produtos:
```sql
SELECT DISTINCT categoria FROM produtos categoria;
 ```

 ## Mostra as categorias do banco de dados produtos em ordem alfabetica:
```sql
...
SELECT nome,preco 
FROM produtos
WHERE categoria='Monitores'; --retorna produtos dentro da categoria Monitores
```
```sql
SELECT nome,preco
FROM produtos
WHERE preco <= 500; --retorna produtos com preços menores que 500
```
*operadores:* <, >, <=, >=.
...
SELECT nome,preco
FROM produtos
WHERE preco BETWEEN 100 AND 1000; --retorna produtos com o preço entre 100 a 1000
```

## Ordena produtos em forma crescente: 
```sql
SELECT nome,preco
FROM produtos
ORDER BY preco;
...
## Ordena produtos em forma decrescente: 
```sql
SELECT nome,preco
FROM produtos
ORDER BY preco DESC;
...
## Atividade Análise de Dados (Livraria):

### Criação do Banco de dados chamado Livraria:
Dentro do usuário postgres, foi usado os comandos `CREATE DATABASE livraria;` para criar o banco de dados, e `\l`para listar os bancos de dados já criados e conferir se 'livraria' está lá.
